...
export function loadPattern(A, pattern, cx, cy, scale = 1) {
    ...
    for (let y = 0; y < sizeY; y++) {
        for (let x = 0; x < sizeX; x++) {
            ...
            if (tx >= 0 && tx < A[0].length && ty >= 0 && ty < A.length) {
                if (Array.isArray(cell)) {
                    A[ty][tx] = cell.reduce((sum, v) => sum + v, 0);  // Se for vetor (pacman atual), pega a soma das camadas
                } else {
                    
                    A[ty][tx] = cell;  // Se for número (orbium)
                }
            }
        }
    }
